...
function WorkshopsInner() {
  ...
  return (
    <div dir={dir} style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text-primary)" }}>
      ...
      <section style={{ padding: "40px 24px 28px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 14, color: "var(--text-tertiary)", margin: "0 0 20px" }}>
            {tr.workshopsSubtitle}
          </p>

          <div style={{
            display: "flex", alignItems: "center", gap: 10, padding: "10px 14px",
            background: "var(--bg-card)", borderRadius: "var(--radius-md)",
            border: "1.5px solid var(--border-strong)", maxWidth: 480,
          }}>
      ...
      <section style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          {filtered.length === 0 ? (
            ...
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
              {filtered.map(shop => {
                ...
                return (
                  <div key={shop.id} className="warsha-card" style={{ padding: 18, position: "relative" }}>
                    ...
                    <div style={{ display: "flex", gap: 7 }}>
                      {shop.phone ? (
                        <button className="warsha-btn-primary"
                          style={{ flex: 1, padding: "8px 0", fontSize: 12, fontFamily: "inherit" }}
                          onClick={() => window.open(`https://wa.me/${shop.phone!.replace(/\D/g,"")}`, "_blank")}>
                          {tr.btnWhatsApp}
                        </button>
                      ) : (
                        <button className="warsha-btn-ghost"
                          style={{ flex: 1, padding: "8px 0", fontSize: 12, fontFamily: "inherit", cursor: "not-allowed", opacity: 0.5 }}
                          disabled>
                          {lang === "ar" ? "الرقم غير متاح" : "No contact yet"}
                        </button>
                      )}
